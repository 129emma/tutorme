...
function renderingOneweek(date,username) {
    // var listing =[];
    var today;
    const tutor = {
        "userName": [username],
    ...
    const promise = new Promise(function (resolve, reject) {
        connectNow.connect(function (err) {
            ...
            connectNow.query("SELECT timeStart FROM tableTime Where timeStart>=? AND timeStart<=? AND username=?", [startOfWeek, endOfWeek,username], function (err, result) {
                connectNow.end();
                console.log('Yeah');
                if (err) {
                    connectNow.end();
...
module.exports = {Threeweeks: preRendering,Oneweek: renderingOneweek};